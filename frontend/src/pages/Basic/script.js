  ...
  async function main() {
    ...
    function start() {
      if (isStarted === false) {
        ...
        if (qnumber === 10 && word_num >= 12) {
          document.getElementById("question2").textContent = questions[word_num]
        }
        // スペースが押されたら、時間計測
        isStarted = true
        timerId = setInterval(() => {
          ...
          if (qnumber === 10 && word_num >= 6) {
            document.getElementById("remaining-time2").textContent = timeLimit - time + "秒"
          }
    ...
    const observer = new MutationObserver(() => {
      ...
      if (qnumber === 10 && word_num >= 7) {
        document.getElementById("progress-number2").textContent = word_num + 1 + "/" + questions.length + "問"
      }
      ...
      if (qnumber === 10 && word_num >= 11) {
        document.getElementById("answered2").textContent = questions[word_num].slice(0, cnt)
      }
      if (qnumber === 10 && word_num >= 12) {
        document.getElementById("question2").textContent = questions[word_num].slice(cnt)
      }
    })
